...
(() => {
  ...
  function installTitlebar(win) {
    ...
    const mount = () => {
      ...
      root.querySelectorAll("button[data-action]").forEach((button) => button.addEventListener("click", () => nativeAction(win, button.dataset.action)));
    ...
    win.addEventListener("keydown", (event) => {
      ...
      if (event.key === "ArrowLeft") {
        event.preventDefault();
        nativeAction(win, "back");
      }
      if (event.key === "ArrowRight") {
        event.preventDefault();
        nativeAction(win, "forward");
      }
    }, true);
  }
